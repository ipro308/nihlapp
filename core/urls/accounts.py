...
urlpatterns = patterns('',
    ...
    (r'^/update/(?P<object_id>\d+)/?$', 'django.views.generic.create_update.update_object', dict(model = User, login_required = True, post_save_redirect='/accounts/confirm/create/%{object_id)s/')),
    (r'^/detail/(?P<object_id>\d+)/?$', 'django.views.generic.list_detail.object_detail', 
        dict(queryset = User.objects.all())),
  #  (r'^/update/(?P<object_id>\d+)/?$', 'django.views.generic.create_update.update_object',dict(model = User, login_required = True)),
    ...
    (r'^/confirm/create/(?P<object_id>\d+)/?$', 'nihlapp.core.views.users.confirm'),
)
